...
class WorkspaceResolver {
    constructor(platformResolver) {
        this.workspaceCache = new Map();
        this.platformResolver = platformResolver || new PlatformPathResolver_1.PlatformPathResolver();
    }
    /**
     ...
     */
    async discoverWorkspaceDatabases() {
        try {
            const storageResult = await this.platformResolver.getVSCodeStoragePath();
            if (!storageResult.isValid || !storageResult.path) {
                return {
     ...
     */
    async validateDatabasePath(databasePath) {
        try {
